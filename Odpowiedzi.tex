		...
		\newpage\section{Inżynieria Oprogramowania}
			...
			\newpage\subsection{Wzorce projektowe programowania obiektowego.}
                ...
                \begin{itemize}
                    ...
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/facade.jpg}
                        \caption{Diagram klas dla wzorca fasady}
                        \label{fig:twojastara22}
                    \end{figure}
                    \item \textbf{Kompozyt} może zostać wykorzystany, kiedy chcemy wymodelować hierarchię obiektów w postaci drzewa. Drzewo to będzie składać się z obiektów klasy \textit{Component}, po których dziedziczą obiekty klasy \textit{Leaf} (będące finalnym elementem w gałęzi drzewa), oraz obiekty klasy \textit{Composite}, agregujące kolejne obiekty Component. Interfejs Component definiuje też metodę \textit{Operation()}, która jest implementowana w różny sposób w klasach Leaf i Composite - Composite deleguje wykonanie operacji do agregowanych do niego obiektów, natomiast Leaf wykonuje jakąś konkretną operację. Zastosowanie tego wzorca umożliwia też kodowi klienckiemu na dostęp do struktury drzewa poprzez interakcję z korzeniem.
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/composite.jpg}
                    ...
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/most.jpg}
                        \caption{Diagram klas dla wzorca mostu}
                        \label{fig:twojastara25}
                    \end{figure}
                    \item \textbf{Pełnomocnik} (proxy) to model pozwalający na zastąpienie obiektu danej klasy jego pełnomocnikiem. Obiekty \textit{RealSubject} i \textit{Proxy} implementują wspólny interfejs \textit{Subject}. Obiekt Proxy agreguje obiekt RealSubject i to on zarządza tym obiektem oraz podejmuje decyzje dotyczące tworzenia go oraz przekazania mu kontroli. Pełni w ten sposób funkcję ochronną i kontrolną.
                \end{itemize}
                \textbf{Wzorce operacyjne}
                \begin{itemize}
                    ...
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/iterator.jpg}
                        \caption{Diagram klas dla wzorca iteratora}
                        \label{fig:twojastara12}
                    \end{figure}
                    \item \textbf{Mediator} to wzorzec definiujący komunikację pomiędzy wieloma obiektami implementującymi ten sam interfejs. Zamiast definiować zależność obiektów od siebie, posiadają one referencję do obiekty \textit{Mediator}, który odpowiada za realizację komunikacji pomiędzy nimi.
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/mediator.jpg}
                        \caption{Diagram klas dla wzorca mediatora}
                        \label{fig:twojastara13}
                    \end{figure}
                    ...
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/2]{images/strategy.jpg}
                        \caption{Diagram klas dla wzorca strategia}
                        \label{fig:twojastara16}
                    \end{figure}
                    \item \textbf{Metoda szablonowa} pozwala na zdefiniowanie szkieletu algorytmu w klasie bazowej \textit{AbstractClass} (funkcja szablonowa), z wyodrębnieniem poszczególnych etapów do abstrakcyjnych funkcji (operacji prymitywnych) jako funkcji abstrakcyjnych, które będą implementowane w klasach dziedziczących \textit{ConcreteClass}. Pozwala to na stworzenie algorytmu, którego niektóre etapy są możliwe do modyfikacji poprzez stworzenie oddzielnych implementacji.
                    \begin{figure}[h]
                        \centering
                        \includegraphics[width=\textwidth/4]{images/template_method.jpg}
                        \caption{Diagram klas dla wzorca metoda szablonowa}
                        \label{fig:twojastara17}
                    \end{figure}
                    \item \textbf{Wizytator} może być używany w sytuacji, gdzie konieczne musi być dokonanie takiej samej operacji na wielu obiektach, które niekoniecznie interpretują ten sam interfejs lub należą do tej samej hierarchii. Obiekty klas realizujących abstrakcję \textit{Visitor} odwiedzają obiekty klas reprezentujących konkretne elementy systemu. Konkretny wizytator prosi konkretny odwiedzany obiekt o akceptację wizyty. Akceptacja polega na wywołaniu przez odwiedzany obiekt metody VisitConcreteElementA() lub VisitConcreteElementB().
                    \begin{figure}[h]
                        \centering
